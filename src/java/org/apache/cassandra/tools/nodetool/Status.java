...
public class Status extends NodeToolCmd
{
    ...
    private <T extends Iterable<U>, U> int findMaxAddressLength(Map<String, T> dcs, Function<U, Integer> computeLength)
    {
        int maxAddressLength = 0;
        Set<U> seenHosts = new HashSet<>();
        for (Map.Entry<String, T> dc : dcs.entrySet())
        {
            for (U stat : dc.getValue())
            {
                if (seenHosts.add(stat))
                    maxAddressLength = Math.max(maxAddressLength, computeLength.apply(stat));
            }
        }
